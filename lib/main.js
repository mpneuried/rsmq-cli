(function() {
  ...
  cli.version("@@version").option("-h, --host <value>", "Redis host").option("-p, --port <n>", "Redis port").option("-n, --ns <value>", "RSMQ namespace").option("-t, --timeout <n>", "Timeout to wait for a redis connection").option("-q, --qname <n>", "RSMQ queuename").option("-g, --group [value]", "Client config profile group");
  ...
  commands = [
    {
      ...
      action: function(type, _n, _v, options) {
        ...
        _group = options.group || options.parent.group;
        if (type === "set") {
          cnf.setConfig(_n, _v, _group, function(err, cnf) {
            if (err) {
              _final(err);
              return;
            }
